function renderProducts(type = null, brand = null) {
  ...
  // lọc theo type (FG / AG / TF)
  if (type) {
    list = list.filter((p) => (p.type || "").toUpperCase() === type);
  }

  // lọc theo brand (NIKE / ADIDAS / ...)
  if (brand) {
    list = list.filter((p) => (p.brand || "").toUpperCase() === brand);
  }
